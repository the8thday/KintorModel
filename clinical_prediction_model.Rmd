...
df1 <- df1 %>% 
  ...
  mutate(race = factor(race),
         across(c(ht, ptl, smoke, ui), as.factor)
         # ftv = factor(ftv)
         )
```
...
```{r}
model_glm <- stats::glm(low ~ age + ftv + ht + lwt + ptl + smoke + ui + race1 + race2, 
                        data = df1, family = "binomial")

model_glm2 <- glm(low ~ age + ftv + ht + lwt + ptl + smoke + ui + race, 
                  data = df1, family = "binomial")
...
```{r}
# assumption
# 奇怪以前是支持glm类的呀。。搞半天似乎是我自己模型的问题。。。在经过不断debug后，竟然是vst不能设置成因子
...
The default predictions are on the scale of the log odds. These predictions are also available through the type = "link", type= 'response' probability scale.
...
get_logistic_error(model_glm, data = df1, 
...
```{r}
modelbased::estimate_means(model_glm2, at = c('race'))
```
