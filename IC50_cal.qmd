...
*模型判断：*如何判断一个curve模型呢？`drc`包似乎没有提供类似Prism R2的结果。而且在非线性模型中，R2不再是线性模型中x对
...
fitted_curve2 <- drc::drm(
  ...
  data = d_long,
  # fct = LL.4(names = c("hill", "min_value", "max_value", "ec_50"))
  fct = drc::LL.4(fixed = c(1,NA,NA,NA))
  )
...
对于连续变量结局模型的评价，lack-of-fit的P值大于0.05表明模型还不错，不过Prism有提供R2值，目前`drc`包还没有找到计算R2的方法。
...
mselect(fitted_curve, fctList = list(W1.3(fixed=c(NA, 100, NA)),
                                     W1.4(), 
